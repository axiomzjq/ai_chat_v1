...
### 项目定位
**ToB 创始人 IP 深度定制系统** - 基于 React + Express + PostgreSQL + 智谱 AI 的五步工作流平台
...
| 2026-06-25 | 添加隐私政策/服务条款/技术支持弹窗 | 合规要求 |
| 2026-06-25 | 彻底移除语音输入功能（STT+TTS） | 简化交互，降低复杂度 |
| 2026-06-25 | 访谈结束改为 AI 主动触发 | 移除 15 轮限制，更自然结束 |
| 2026-06-25 | 管理员用户解锁所有步骤导航 | 方便管理员快速访问各功能 |
| 2026-06-28 | 添加选题页面（Topic Page） | 定位与文案之间的选题规划环节 |
| 2026-06-28 | 选题页面 UI：文件夹样式标签页 | 更直观的阶段切换体验 |
| 2026-06-28 | 选题卡片压缩 + 黑色实心按钮 | 提升页面信息密度和视觉一致性 |
| 2026-07-05 | 提示词架构重构为步骤化文件引用 | 基础提示词精简，详细方法论从 txt 加载 |
| 2026-07-05 | 多用户数据隔离修复 | localStorage 按 userId 隔离，防止数据泄漏 |
| 2026-07-05 | 步骤解锁逻辑修复 | topicPool 传递错误导致文案页始终锁定 |
| 2026-07-05 | 选题页空状态提示 | 未生成时显示引导按钮，不再默认显示 Demo |
| 2026-07-05 | 定位方案 3 版独立输出 | prompt 格式要求 + 正则修复 |
| 2026-07-05 | 调试模式增加清除本地缓存按钮 | 方便测试时清除 localStorage |
| 2026-07-09 | 移除文案顾问对话模式 | 选中选题直接生成文案，跳过思路整理对话 |
| 2026-07-09 | 通用 JSON 解析器提取 | parseJsonResponse 统一处理选题/文案解析，4 层策略 |
| 2026-07-09 | localStorage 用户数据缓存 | 登录后缓存该用户的访谈/定位/选题/文案数据到 localStorage，减少重复 API 请求 |
| 2026-07-18 | 增加超级管理员(superadmin) | ADMIN_PHONE 控制，可管理所有用户；普通管理员(admin)可管理普通用户但不可操作超级管理员 |
| 2026-07-18 | Header 步骤导航移到标题栏 | 紧凑圆点+文字，降低主体垂直空间 |
| 2026-07-18 | 对话智能滚动锁定 | 用户向上滚动查看历史时不再强制拉回底部 |
| 2026-07-18 | Header 显示当前登录账号 | 退出按钮旁显示手机号/邮箱 |
| 2026-07-18 | 上传文件 accept 修复 | 移除不支持的图片/PDF/视频格式 |
| 2026-07-19 | 新增 .pptx 文件上传支持 | JSZip 解压提取文本，支持 PPT 参考资料上传 |
...
| **选题** | 选题顾问 | 完成访谈 | 访谈完成后解锁 |
| **文案** | 文案顾问 | 完成访谈并生成选题池 | 选题完成后解锁 |
| **历史** | — | 始终可进 | 始终可用 |

**关键规则：**
- 访谈第 20 轮后解锁"生成深度报告"按钮
- 报告生成手动触发（非自动）
- 步骤导航已移到标题栏（紧凑圆点 + 常驻文字标签，`lg:` 以上显示）
- 选题页面位于定位与文案之间，展示 4 个阶段的选题规划池
- 选题页面 UI：文件夹样式标签页 + 阶段简介面板 + 压缩选题卡片
- 文案页面直接生成（已移除文案顾问对话模式），选中选题后 AI 直接输出 3 个标题 + 1 篇口播文案

### 对话智能滚动

```typescript
const isNearBottomRef = useRef(true);
const checkNearBottom = (el, threshold = 80) => {
  return el.scrollHeight - el.scrollTop - el.clientHeight < threshold;
};
```

**行为：** 新消息到达时，仅在用户已在底部附近（< 80px）时才自动滚动。用户向上滚动查看历史时保持位置，滚回底部后恢复自动跟随。
...
|------|---------|------|
| `.txt` / `.md` | 直接读取文本 | 推荐 |
| `.docx` | `mammoth` 提取纯文本 | 推荐 |
| `.doc` | `mammoth` 提取纯文本 | 旧格式，可能失败 |
| `.pptx` | `JSZip` 解压 → 提取 XML `<a:t>` 标签 | 仅提取文字，格式丢失 |
| `.xlsx` / `.xls` | `XLSX` 读取第一个 sheet | 仅取第一个 sheet |

**限制：** 单文件 ≤ 2MB，内容截断到 8000 字符。`.pdf`、图片、视频不支持。
...
| `.txt` / `.md` | 直接读取文本 |
| `.docx` | `mammoth` 提取纯文本 |
| `.pptx` | `JSZip` 解压 → 提取 XML `<a:t>` 标签 |
| `.xlsx` / `.xls` | `XLSX` 读取第一个 sheet |

知识库上传无大小限制，AI 自动整理内容结构。

### 三级权限体系

| 角色 | 数据库 `role` | 识别方式 | 默认额度 | 订阅天数 |
|------|-------------|---------|---------|---------|
| **超级管理员** | `superadmin` | `ADMIN_PHONE` 环境变量手机号匹配 | 999,999,999 | 99,999 天 |
| **普通管理员** | `admin` | 预创建或超级管理员提升 | 999,999,999 | 99,999 天 |
| **普通用户** | `user` | 默认 | 100,000 | 7 天 |

**关键规则：**
- 超级管理员和普通管理员都解锁所有步骤导航
- 普通管理员只能管理普通用户（查看、编辑额度、切换角色），无法操作超级管理员
- 超级管理员可管理所有用户（包括其他管理员）
- 管理员对话界面和普通用户一致，无特殊功能
...
| 文案 | `COPYWRITING_GENERATE_SYSTEM_PROMPT` | `refs/copywriting/客户采访与选题提示词.txt`<br>`refs/copywriting/文案审核提示词.txt` | 访谈结果 + 定位报告 + 选题池 | 文案（3标题 + 1脚本） |
| 信息 | `INFO_SYSTEM_PROMPT` | 无（嵌入访谈流程） | 访谈报告 | 企业/行业分析 |

**Prompt 文件：** `src/lib/prompts.ts`
**参考文件：** `public/refs/` 目录（按步骤分文件夹，浏览器 fetch 加载）
**文档：** `knowledge-base/提示词文件架构.md`

### 通用 JSON 解析器

`parseJsonResponse(aiResponse)` 用于解析 AI 返回的 JSON，支持 4 层策略：

| 策略 | 方法 | 适用场景 |
|------|------|---------|
| 1 | `JSON.parse` 直接解析 | AI 返回纯 JSON |
| 2 | 正则提取 ` ``` ` 代码块 | AI 用 Markdown 包裹 JSON |
| 3 | 提取第一个 `{` 到最后一个 `}` | JSON 前后有废话 |
...
**角色识别：**
- **超级管理员**：`ADMIN_PHONE` 环境变量手机号匹配，数据库 `role = 'superadmin'`
- **普通管理员**：管理后台预创建或超级管理员提升，数据库 `role = 'admin'`
- **普通用户**：默认角色，数据库 `role = 'user'`

**权限层级：**
```
superadmin（超级管理员）
  └── 可管理所有用户（包括 admin 和 superadmin）
       └── 预创建任意角色用户
admin（普通管理员）
  └── 可管理普通用户（user）
       └── 预创建普通用户
...
登录后，前端会将当前用户的关键数据缓存到 localStorage，减少重复 API 请求：

| 数据类型 | localStorage Key | 说明 |
|---------|-----------------|------|
| 访谈消息历史 | `founder_ip_history_{userId}` | 访谈页对话消息 |
| 访谈结果 | `result_interview_report_{userId}` | 访谈报告 |
| 定位报告 | `result_positioning_report_{userId}` | 定位报告 |
| 选题池 | `result_topic_pool_{userId}` | 选题池 |
| 文案输出 | `copywritingOutput` (state) | 文案结果（内存中） |
| UI 状态 | `ai_chat_ui_state_{userId}` | 页面 UI 状态 |

**关键特性：**
- **用户隔离**：所有 key 通过 `getUserScopedKey(baseKey, userId)` 添加 `_{userId}` 后缀
- **登出清除**：`handleLogout` 清除该用户的全部 localStorage 数据
- **双重持久化**：关键结果同时保存到后端（`user_profiles` 表）和 localStorage
